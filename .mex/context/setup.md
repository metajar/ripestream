...
- `CLICKHOUSE_PASSWORD='<secret>' R2_ACCOUNT_ID='<id>' R2_ACCESS_KEY_ID='<key>' R2_SECRET_ACCESS_KEY='<secret>' R2_BUCKET='<bucket>' docker compose -f docker-compose.prod.yml up -d --build` — build and start the full production stack (dashboard on host port 3000)
...
## Common Issues
- **Dokploy deployment**: Select `docker-compose.prod.yml`, set the required `CLICKHOUSE_PASSWORD`, `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, and `R2_BUCKET`, then route the domain to service `ripestream` on container port `8080`. Only the dashboard is published on host port 3000; the databases remain private.
